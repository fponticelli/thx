...
class Tree<T> extends Hierarchy<T>{
	var _separation:TreeNode<T>->TreeNode<T>->Int;
	var _size : Array<Float>;
	var _links : Array<Node<T>>->Array<Link<T>>;
	var _treeHierarchy : T->Int->Array<TreeNode<T>>;
/*	var _callback;*/
	public function new(){
		super();		
	}
	...
	function secondWalk(node : TreeNode<T>, x) {
	...
	public function tree(d, i) {
		var nodes = _treeHierarchy(d, i),
		root = nodes[0];
	...
	public static function treeLinks<T>(nodes) {
	...
	public static function visit<T>(node:TreeNode<T>, previousSibling:TreeNode<T>, callbackf:TreeNode<T>->TreeNode<T>->Void) {
		...
		if (children != null) {
			...
			while (++i < n) {
				child = children[i];
				visit(child, previousChild, null);
				previousChild = child;
			}
		}
